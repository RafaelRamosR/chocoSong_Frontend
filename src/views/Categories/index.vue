                ...
                <template #tbody>
                    <tr v-for="dato in datos" :key="dato.id">
                        ...
                        <td class="flex items-center">
                            <RouterLink :to="{name:'categories.edit',params:{id:dato.id}}" class="mr-3">
                                <IconEdit/>
                            </RouterLink>
                            <RouterLink :to="{name:'categories.show',params:{id:dato.id}}" class="mr-3">
                                <IconShow class="mr-2"/>
                            </RouterLink>
...
export default ({
    ...
    methods:{
        ...
        forceDelete(){
            if (this.id) {
                categoriesServices.delete(this.id)
                .then((response)=>{
                    this.showAlert=false
                    this.getCategories()
                })
