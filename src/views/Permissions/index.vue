            ...
            <Table>
                <template #title>
                    <div class="flex justify-end">
                        <RouterLink :to="{name:'permissions.create'}">
                ...
                <template #tbody>
                    <tr v-for="dato in datos" :key="dato.id">
                        ...
                        <td class="flex items-center">
                            <RouterLink :to="{name:'permissions.edit',params:{id:dato.id}}" class="mr-3">
                                <IconEdit/>
                            </RouterLink>
                            <RouterLink :to="{name:'permissions.show',params:{id:dato.id}}" class="mr-3">
                                <IconShow class="mr-2"/>
                            </RouterLink>
...
export default ({
    ...
    methods:{
        ...
        forceDelete(){
            if (this.id) {
                permissionsServices.delete(this.id)
                .then((response)=>{
                    this.showAlert=false
                    this.getCategories()
                })
